...
from PyQt5.QtWidgets import (
    QApplication, QWidget, QTabWidget, QBoxLayout, QHBoxLayout, QVBoxLayout, QGroupBox,
    QSystemTrayIcon, QMenu, QAction, QStyle, QPushButton, QSpinBox, QScrollArea,
    QLabel, QColorDialog, QMessageBox, QComboBox, QCheckBox, QFormLayout
)
from PyQt5.QtCore import Qt, QTimer
from PyQt5.QtGui import QFont, QIcon, QColor, QPalette, QCloseEvent
import json
import copy
...
class NetSpeedOverlay(QWidget):
    def __init__(self, config: dict[str, Any]):
        ...
        self.box_layout.setContentsMargins(6, 6, 6, 6)
        ...
        self.box_layout.addWidget(self.down_label)
        self.box_layout.addWidget(self.up_label)
        ...
        self.show()
        self.make_click_through(self.config["behavior"].get("click_through", True))
    ...
    def format_speed(self, bytes_per_sec: float, direction: str) -> str:
        ...
        direction: 'download' or 'upload'
        """
        unit_mode: str = self.config["appearance"].get(f"{direction}_unit_mode", "Auto")
    ...
    def update_speed(self):
        ...
        try:
            ...
            # Recover safely
            logging.exception(f"Network counter error: {e}")
            return
    ...
    def apply_settings(self, cfg):
        ...
        if appearance["layout"] == "vertical":
            self.box_layout.setDirection(QBoxLayout.TopToBottom)
        else:
            self.box_layout.setDirection(QBoxLayout.LeftToRight)
...
class SettingsWindow(QWidget):
    def __init__(self, overlay: NetSpeedOverlay, config: dict[str, Any]):
        ...
        self._threshold_signals_connected = False

        self.setWindowTitle("Network Speed Monitor Properties")
        self.setWindowIcon(QIcon(resource_path("up_down_icon.ico")))
    ...
    def appearance_tab(self):
        ...
        # ---- Very High (color only) ----
        row = QHBoxLayout()

        label = QLabel("Very high")
        label.setFixedWidth(90)

        spacer = QLabel("")  # no limit
        ...
        color_btn.setFixedSize(60, 24)
        color_btn.setStyleSheet(
            f"background-color: {self.config['thresholds']['download']['very_high']['color']};"
        )
        color_btn.clicked.connect(lambda _, k="very_high": self.pick_color("download", k))
        ...
        self.dl_color_btns["very_high"] = color_btn
        vbox_layout_download.addLayout(row)
        # ---- End: Very High (color only) ----
        ...
        color_btn.setFixedSize(60, 24)
        color_btn.setStyleSheet(
            f"background-color: {self.config['thresholds']['upload']['very_high']['color']};"
        )
        color_btn.clicked.connect(lambda _, k="very_high": self.pick_color("upload", k))
        ...
        self.ul_color_btns["very_high"] = color_btn
        vbox_layout_upload.addLayout(row)
        # ---- End: Very High (color only) ----
        ...
        if not self._threshold_signals_connected:
            for key, spin in self.dl_thresholds.items():
                spin.valueChanged.connect(
                    partial(self.sync_upload_threshold, key)
                )

            self._threshold_signals_connected = True
    ...
    def behavior_tab(self):
        ...
        update_layout.addRow("Refresh interval (250–5000 ms):", self.refresh_spin)
    ...
    def sync_upload_threshold(self, key, value):
        if self.link_upload_cb.isChecked():
            self.ul_thresholds[key].setValue(value)
...
class TrayController:
    def __init__(self, app: QApplication, overlay: NetSpeedOverlay):
        ...
        icon = QIcon(resource_path("up_down_icon.ico"))
    ...
    def open_settings(self):
        ...
        self.settings.startup_cb.setChecked(is_startup_enabled())
...
if __name__ == "__main__":
    ...
    app.setWindowIcon(QIcon(resource_path("up_down_icon.ico")))
